<template>
  <div class="review">
    ...
    <!-- 轮播图层  -->
    <div class="banner">
      <img src="./images/TestPageBanner.jpg" />
    </div>
    <!-- 轮播图层结束  -->
      ...
      <ul>
        <li v-show="index === i" v-for="(k, i) in questionAll" :key="i">
          ...
          <div class="item">
            ...
            <!-- 题目注释结束-->
            <div @click="collect(k.id)" class="collect">
  ...
  methods: {
    ...
    getQuestion() {
      ...
      for (let i = 0; i < this.collectAll.length; i++) {
        this.axios
          .get("http://m.jxedt.com/mnksnew/g.asp?id=" + this.collectAll[i] + "")
    ...
    getQuestionCollect() {
      if (localStorage.getItem("错题本的题目")) {
        this.collectAll = JSON.parse(localStorage.getItem("错题本的题目"));
      }
                ...
                for (let i = 0; i < this.collectAll.length; i++) {
                  if (id == this.collectAll[i]) {
                    ...
                    localStorage.setItem(
                      "错题本的题目",
                      JSON.stringify(this.collectAll)
                    );
                  }
                }
    ...
    collect(id) {
      ...
      for (let i = 0; i < this.collectAll.length; i++) {
        if (id == this.collectAll[i]) {
          this.collectAll.splice(i, 1);
          localStorage.setItem("错题本的题目", JSON.stringify(this.collectAll));
        }
      }
