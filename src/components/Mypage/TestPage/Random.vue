...
export default {
  ...
  created() {
    this.axios.get("http://m.jxedt.com/mnksnew/g.asp?id=1").then((res) => {
      ...
      if (localStorage.getItem("随机联系的记录表")) {
        this.$dialog
          .confirm({
            title: "开始练习",
            message:
          ...
          .catch(() => {
            localStorage.removeItem("随机联系的记录表");
            location.reload();
  ...
  mounted() {
    if (localStorage.getItem("随机联系的记录表")) {
      this.questionAll = JSON.parse(localStorage.getItem("随机联系的记录表"));
  ...
  methods: {
   ...
   if(this.pattern){
      ...
      if (this.pattern) {
        ...
        //   this.falseNum++;
        // }
        localStorage.setItem("随机联系的记录表", JSON.stringify(this.questionAll));
